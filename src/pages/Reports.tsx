...
const Reports: React.FC = () => {
  ...
  const [billSettings, setBillSettings] = useState<{
    ...
    whatsappBillShareEnabled?: boolean;
    gstin?: string;
  } | null>(null);
  ...
  const handleWhatsAppShareBill = async (bill: Bill, mode: 'text' | 'image' = 'text') => {
    ...
    if (mode === 'text') {
      if (!whatsappPhone.trim()) {
        toast({ title: "Enter phone number", description: "Please enter customer's phone number for text share", variant: "destructive" });
        return;
      }
    ...
    if (mode === 'image') {
      ...
      const billData: BillImageData = {
        ...
        fontFamily: settings?.billFontFamily || getStoredBillFont(bill.branch_id || undefined),
        fontScale: settings?.billFontScale ? Number(settings.billFontScale) : getStoredBillFontScale(bill.branch_id || undefined),
        footerMessage: settings?.billBottomText || getStoredFooterMessage(bill.branch_id || undefined)
      };
    ...
    if (savedSettings) {
      try {
        ...
        setBillSettings({
          ...
          whatsappBillShareEnabled: parsed.whatsappBillShareEnabled !== false,
          gstin: parsed.gstin || ''
        });
