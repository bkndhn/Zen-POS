...
export const CompletePaymentDialog: React.FC<CompletePaymentDialogProps> = ({
  ...
  const handleCompletePayment = (printAction: 'print' | 'no-print' = 'print') => {
    ...
    if (filteredPaymentAmounts['Khata (Credit)'] > 0) {
      if (!customerMobile.trim() || !customerName.trim()) {
        toast({
          title: "Customer Required",
          description: "Khata (Credit) payments require a valid customer name and mobile number.",
          variant: "destructive"
        });
        return;
      }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md h-[100dvh] sm:h-[95vh] flex flex-col p-0 gap-0 overflow-hidden border-2 border-primary/20 sm:rounded-lg rounded-none bg-background text-foreground">
        ...
        <div className="flex-1 overflow-hidden p-2 flex flex-col gap-1.5">
          ...
          {/* Customer Details & WhatsApp Share */}
          {(whatsappEnabled || paymentAmounts['Khata (Credit)'] > 0) && (
            <div className={cn("rounded-lg p-2 flex-shrink-0", whatsappEnabled ? "bg-gradient-to-r from-green-50 to-teal-50 dark:from-green-950/20 dark:to-teal-950/20" : "bg-muted/30 border border-muted-foreground/20")}>
              <div className="flex items-center gap-2 mb-1.5">
                {whatsappEnabled ? (
                  <>
                    <svg viewBox="0 0 24 24" className="w-4 h-4 text-[#25D366] flex-shrink-0" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" /></svg>
                  ...
                  <span className="font-semibold text-xs text-muted-foreground flex-1">Customer Details</span>
                )}
              </div>
        ...
        <div className="border-t border-primary/10 bg-muted/40 p-3 pb-3.5 flex-shrink-0 space-y-3">
          ...
          <div className="flex items-center justify-between bg-white dark:bg-zinc-950 p-2 rounded-xl border border-input shadow-sm">
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold text-muted-foreground">SPLIT BILL:</span>
              <button 
                onClick={() => setSplitWays(w => Math.max(1, w - 1))}
                className="w-6 h-6 flex items-center justify-center rounded bg-muted hover:bg-muted/80 text-foreground font-bold"
              >
                -
              </button>
              <span className="text-sm font-bold w-4 text-center">{splitWays}</span>
              <button 
                onClick={() => setSplitWays(w => w + 1)}
                className="w-6 h-6 flex items-center justify-center rounded bg-muted hover:bg-muted/80 text-foreground font-bold"
              >
                +
              </button>
            ...
            {splitWays > 1 && (
              <div className="text-xs font-bold bg-primary/10 text-primary px-2 py-1 rounded-lg">
                ₹{(total / splitWays).toFixed(2)} / person
              </div>
            )}
          </div>
          ...
          <div className="grid gap-2" style={{ gridTemplateColumns: `repeat(${Math.min(paymentTypes.length + 1, 4)}, minmax(0, 1fr))` }}>
            ...
            {khataEnabled && (
              <div className="flex flex-col items-center">
                <Button 
                  variant={paymentAmounts['Khata (Credit)'] > 0 ? "default" : "outline"} 
                  size="sm" 
                  onClick={() => setPaymentAmounts({ 'Khata (Credit)': total })} 
                  className={cn(
                    "capitalize text-xs h-11 sm:h-12 w-full font-bold mb-1 rounded-xl transition-all",
                    !(paymentAmounts['Khata (Credit)'] > 0) && "border-dashed border-primary text-primary hover:text-primary-foreground hover:bg-primary"
                  )}
                >
                  Khata (Credit)
                </Button>
                <Input 
                  type="number" 
                  value={paymentAmounts['Khata (Credit)'] || 0} 
                  onChange={(e) => handlePaymentAmountChange('Khata (Credit)', Number(e.target.value))} 
                  className="h-10 text-sm text-center font-bold w-full rounded-xl bg-white dark:bg-zinc-950 border border-input" 
                  min="0" 
                  step="0.01" 
                />
