-- Add unit and base_value to bill_items if not present
ALTER TABLE public.bill_items ADD COLUMN IF NOT EXISTS unit text;
ALTER TABLE public.bill_items ADD COLUMN IF NOT EXISTS base_value numeric DEFAULT 1;

-- Update secure_create_bill to respect cart item line total, base_value, unit, item_name snapshot, and local date
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO bill_items (
      ...
      v_taxable_amount, v_tax_amount, v_billing_type, v_item_name,
      coalesce(v_cart_unit, v_db_selling_unit, v_db_unit, 'pcs'),
      coalesce(v_cart_base_value, v_db_base_value, 1)
    );

    IF v_billing_type != 'calci' AND v_item_id IS NOT NULL THEN
