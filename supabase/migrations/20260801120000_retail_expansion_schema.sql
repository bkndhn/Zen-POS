-- Database Schema Expansion (Retail Core)
-- Creates departments, brands, item_variants and alters items and bill_items.
...
CREATE POLICY "Enable read for authenticated users" ON public.departments FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "Enable all for admin" ON public.departments FOR ALL USING (
  admin_id IN (
...
CREATE POLICY "Enable read for authenticated users" ON public.brands FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "Enable all for admin" ON public.brands FOR ALL USING (
  admin_id IN (
    SELECT admin_id FROM profiles WHERE user_id = auth.uid()
    UNION
    SELECT id FROM profiles WHERE user_id = auth.uid()
...
-- RLS for item_variants
ALTER TABLE public.item_variants ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Enable read for authenticated users" ON public.item_variants FOR SELECT USING (auth.role() = 'authenticated');
CREATE POLICY "Enable all for admin" ON public.item_variants FOR ALL USING (
  EXISTS (
    SELECT 1 FROM items
    WHERE items.id = item_variants.item_id
    AND items.admin_id IN (
      SELECT admin_id FROM profiles WHERE user_id = auth.uid()
      UNION
      SELECT id FROM profiles WHERE user_id = auth.uid()
    )
...
-- Update secure_create_bill to handle variant line items
...
BEGIN
  ...
  LOOP
    ...
    ELSE
      ...
      -- Handle variant pricing override
      IF v_variant_id IS NOT NULL THEN
        SELECT rsp INTO v_rsp FROM item_variants WHERE id = v_variant_id;
        IF v_rsp IS NOT NULL AND v_rsp > 0 THEN
          v_db_price := v_rsp;
        END IF;
      END IF;
    END IF;
    ...
    INSERT INTO bill_items (
      bill_id, item_id, variant_id, quantity, price, total,
      tax_rate_snapshot, tax_rate, hsn_code, tax_type,
      taxable_amount, tax_amount, billing_type, item_name_override,
      unit, base_value
    ) VALUES (
      v_bill_id, v_item_id, v_variant_id, v_qty, v_db_price, v_line_total,
    ...
    IF v_item_id IS NOT NULL THEN
      ...
      ELSIF EXISTS (SELECT 1 FROM recipes WHERE item_id = v_item_id) THEN
