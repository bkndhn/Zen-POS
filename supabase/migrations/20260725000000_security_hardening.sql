...
-- Fixes:
-- C1: secure_create_bill missing caller validation (CRITICAL)
-- M3: bill_items INSERT policy too permissive (MEDIUM)

-- ==========================================================
-- FIX C1: Add caller validation to secure_create_bill
-- Ensures auth.uid() matches the created_by in the payload
-- to prevent bill attribution spoofing.
-- ==========================================================
...
BEGIN
  ...
  -- *** SECURITY FIX C1: Caller validation ***
  -- Verify that the authenticated user (auth.uid()) is authorized to create this bill.
  -- The created_by must be the caller's own profile, OR the caller must be the admin
  -- of the user specified in created_by.
  ...
  -- Check: created_by must be the caller OR caller must be the admin of the created_by user
  IF v_created_by != auth.uid() THEN
    -- Check if caller is admin of the created_by user
    IF NOT EXISTS (
  ...
  -- Also validate that the admin_id in the payload matches the caller's tenant
  IF v_caller_admin_id IS NOT NULL THEN
    -- Caller is a sub-user, admin_id must match their parent
    IF v_admin_id != v_caller_admin_id THEN
      RAISE EXCEPTION 'Unauthorized: admin_id does not match your tenant';
    END IF;
  ELSE
    -- Caller is an admin, admin_id must be their own profile id
...
-- ==========================================================
-- FIX M3: Tighten bill_items INSERT policy
-- Replace the permissive WITH CHECK (true) with proper
-- admin_id scoping via the parent bill's admin_id.
-- ==========================================================

-- Drop the old permissive policy if it exists
DO $$
BEGIN
  -- Try to drop the old permissive INSERT policy
  BEGIN
  ...
  BEGIN
    DROP POLICY IF EXISTS "Users can insert bill_items" ON public.bill_items;
  EXCEPTION WHEN OTHERS THEN NULL;
  END;
...
-- Users can only insert bill_items for bills that belong to their tenant
CREATE POLICY "bill_items_insert_tenant_scoped" 
  ...
  WITH CHECK (
    EXISTS (
      ...
      AND bills.admin_id = get_user_admin_id()
    )
  );
